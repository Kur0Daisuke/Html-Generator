...
def choose():
    ...
    if folder_selected != "":
        createButton.configure(command=lambda: [createFiles(folder_selected), messagebox.showinfo("Success", "Successfully generated!!")])
    else:
        createButton.configure(command=lambda: messagebox.showerror("Error", "Warning : \n Reason : No Files Selected"))
...
createButton = Button(can,text="create", command=lambda: messagebox.showerror("Error", "Warning : \n Reason : No Files Selected"), height=1, width=15, bd=1)
createButton.place(x=140,y=100)
...
Button(can,text="Choose Folder", command=lambda: choose(), height=1, width=15, bd=1).place(x=30,y=100)
